...
- Within one manifest, two keys that differ only in case are rejected: they would share one directory and one registry file on a case-insensitive filesystem. Across installed packages the same collision is caught at discovery: when two packages declare keys that resolve to one directory below the data root, or when two packages register the same `fwl_io.manifests` entry-point name, discovery raises `ManifestConflictError` naming every provider, key or package involved, because fetching either dataset would overwrite the other's and there is no safe resolution. Pin or uninstall all but one of the colliding packages to recover.
