...
- Within one manifest, two keys that differ only in case are rejected: they would share one directory and one registry file on a case-insensitive filesystem. Across installed providers the same collision is caught at discovery: when two providers declare keys that resolve to one directory below the data root, or when one `fwl_io.manifests` entry-point name is registered more than once, discovery removes every provider involved, because fetching either dataset would overwrite the other's and there is no safe resolution. A provider in neither kind of collision stays loaded; one whose entry-point name is repeated is removed even when its locations are unique. The reason appears per removed provider in `fwl-io list` (on stderr, with exit status 1), in `check_for(...).manifest_errors`, and in the `fetch_for` failure of a model that needs a dataset from a removed provider; a model that needs none is unaffected. Pin or uninstall all but one of the colliding packages to recover.
