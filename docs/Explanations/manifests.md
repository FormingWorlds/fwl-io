...
Each dataset has a registry file next to its manifest, named `<dotted-key>.registry.txt`, one `<name> <hash>` entry per line in the format pooch reads natively. Registries are generated by `fwl-io sync` and committed; they are never edited by hand. File names may contain forward slashes for files nested below the dataset directory, and are held to the same escape rules as the dataset key.
