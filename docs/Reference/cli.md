...
Two kinds of failure are reported apart from the datasets, because they call for different repairs. `MANIFEST NOT USED` means an installed package's manifest was left out, either because it could not be read or because it conflicts with another installed manifest, so nothing it declares was inspected. `NOT CHECKED` means the manifest was fine but one dataset could not be resolved, most often because its registry has not been generated yet; run `fwl-io sync` for it. Either is on its own enough to fail the check.
