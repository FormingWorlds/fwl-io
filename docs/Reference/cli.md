...
Every present file in the old location that matches its recorded digest moves; an absent file is reported for a later fetch to fill in at the new location. A present file whose contents differ, a legacy tree holding none of the registry's files, or a dataset whose registry has not been generated is reported and left exactly where it is. Verifying first is the point, since moving a stale copy would put it where the fetcher then trusts it. A file already at the new location is not overwritten: one that matches the registry is skipped and its old copy stays where it is, and one that does not blocks the dataset as a mismatch. A corrupt old copy of a file the new location already holds intact still blocks the dataset, and the report says the bad file is only a copy. On a second run after a partial move, a dataset whose old directory holds nothing more to move, and whose new location already holds verified files, is reported as absent with the count already there. It is not a fault, even when the new location also holds a file that differs from the registry; that file is named in the report with the repair `fwl-io check <model>`, then `fwl-io fetch <model>`. An old copy of a file that is already at the new location stays where it is. A file at the new location counts as verified only when it is a regular file with the right digest and every directory between the data root and it is a plain directory, so a symlink at the file or at any directory on the way is never treated as one. A file behind such a symlink is named in the report as not verified. An old copy is called redundant only when it is also reached through plain directories; one that is a symlink, or has a symlinked directory on the way, may lead back to the new location, so the dataset is reported and both are left alone. With no old directory nothing is moved and nothing is called redundant, so the new location counts as current wherever its directories point. Once a dataset's files have moved, the emptied directories are removed, and the walk upward stops at the data root.
...
The move opens every directory without following a symlink, so a dataset is also refused when the old directory is a symlink, when a symlink lies anywhere between the data root and the old directory or the part of the new location that exists, when a registry file or a directory inside a registry name is a symlink, when a path resolves outside the data root, or when a directory on the way cannot be read. A platform that lacks `O_DIRECTORY`, `O_NOFOLLOW`, or a directory-relative open, stat, mkdir or rename, or lacks a stat that leaves a final symlink unfollowed, such as Windows, is refused the same way; unlike deletion, a move needs neither flock nor a symlink-safe `rmtree`. `--dry-run` reports every one of these refusals. It does not check write permission or free space, so a directory this user cannot write still fails the real run, which then puts back what it moved.

A run assumes that no other process renames or replaces directories under the data root while it moves files. Unlike a deletion, a move does not check the fetch lock, so a fetch running at the same time is not detected: do not relocate while a fetch could be running. A file created at a destination between the check that the name is free and the move is overwritten. A move that fails part way puts back the files it already moved, and the datasets after it are still attempted. A dataset whose files cannot be put back is reported as split between the two locations, which needs a person to look at both directories.
...
Exit is 1 when a legacy tree was found and could not be moved, when an installed manifest failed to load or was dropped as a conflict, since that manifest may be the one declaring the dataset a tree still holds, or when the shipped table of old locations could not be read, since without it no dataset has an old location to look at and a run that reported nothing would read like a tidy tree. A tree that was already tidy exits 0. `--dry-run` reports the same plan without moving anything. The equivalent Python entry points are `fwl_io.relocate_all` and `fwl_io.plan_relocations`; `Relocation.files` names the files a move would move: in a plan it is empty unless the state is ready, and the results of `relocate_all` keep it on moved, failed and split entries.
