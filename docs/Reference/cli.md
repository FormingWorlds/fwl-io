...
The plan and the delete happen in two separate calls, so the tree can change between them: a fetch can start, a pin can advance, or a candidate's classification can move between superseded and orphaned. The delete call removes a directory only when the plan listed it as a target and a fresh classification still does, so nothing is deleted that the confirmation did not list. It refuses the whole run when the fresh reference set no longer permits deletion, or when the plan was built for another data root.
